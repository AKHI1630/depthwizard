"""Run full pipeline on all 4 crops and report results."""
import glob
import json
import os
import time
...
crops = sorted(glob.glob("data/crops/crop*.tif"))
print(f"Found {len(crops)} crops")

# Corrected sun from STAC timestamp (pysolar already returns compass bearing)
computed_elev = 74.03
computed_az = 108.65  # ESE, morning sun — NOT the 288.65 from the flipped convention
...
for crop_path in crops:
    ...
    with open(crop_path, "rb") as f:
        resp = requests.post(
            ...
            params={
                "gsd": 0.58,
                "sun_elevation": computed_elev,
                "sun_azimuth": computed_az,
                "sam_points": 8,
            },
    ...
    result = {
        ...
        "sun_source": sun.get("source"),
        "sun_elev": sun.get("elevation_deg"),
    ...
    print(f"  Computed sun az:      {computed_az} deg")
    print(f"  Measured shadow az:   {measured_az}")
    print(f"  Azimuth delta:        {delta} deg")
    print(f"  Consistent:           {consistent}")
    print(f"  Sun used:             elev={sun.get('elevation_deg')} az={sun.get('azimuth_deg')} source={sun.get('source')}")
...
print("SUMMARY TABLE")
print(sep)
hdr = f"{'Crop':<25} {'R':>6} {'OK':>4} {'Delta':>7} {'Bldg':>5} {'Meas':>5} {'Rel':>5} {'Fail':>5} {'Mean_h':>7} {'Time':>6}"
print(hdr)
print("-" * len(hdr))
for r in results:
    ...
    print(
        ...
        f"{r['total']:>5} {r['measured']:>5} {r['relative']:>5} {r['failed']:>5} "
        f"{h_str:>7} {r['wall_s']:>5.0f}s"
    )

# Cross-crop consistency check (Fix 3)
# All crops are from the same acquisition — measured azimuths must agree within 20 deg
print(f"\n{sep}")
...
if len(crop_azimuths) >= 2:
    ...
    for name, az in crop_azimuths:
        print(f"  {name}: measured_az = {az:.1f} deg")
    print(f"  Circular mean:       {mean_az:.1f} deg")
    print(f"  Cross-crop R:        {R_cross:.3f}")
    print(f"  Max pairwise delta:  {max_delta:.1f} deg ({max_pair[0]} vs {max_pair[1]})")

    if max_delta > 20:
        print(f"  VERDICT: UNRELIABLE — max delta {max_delta:.1f} > 20 deg threshold")
        print(f"  Shadow measurement is unreliable for this scene. Use computed sun.")
    else:
...
print(sep)
for r in results:
    print(f"  {r['name']}: source={r['sun_source']}, elev={r['sun_elev']}, az={r['sun_az']}")

print(f"\nComputed sun (from STAC timestamp): elev={computed_elev} deg, az={computed_az} deg (ESE)")
print("Acquisition: 2024-05-29T05:10:55Z, WV02, scene centre 27.74N 85.32E")
print(f"h_min at this elevation: {5.0 * 0.58 * 3.49:.1f}m (buildings shorter than this cannot be measured)")
print("pysolar convention: verified — returns compass bearing (N=0, CW), no +180 needed")
